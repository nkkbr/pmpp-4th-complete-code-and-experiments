...
__global__ void histo_private_kernel(char *data, unsigned int length, unsigned int *histo) {
    ...
    if (blockIdx.x>0){
        ...
        // We use threads in the block to parallelize the merge process for NUM_BINS.
        // In this setting, NUM_BINS is 7, and blockDim.x is typically larger (e.g., 256).
        // Therefore, this loop effectively executes only once for the first 7 threads.
...
int main(int argc, char **argv){
    ...
    unsigned int *h_histo_gpu = (unsigned int *) malloc(NUM_BINS*sizeof(unsigned int)*grid.x); 
    ...
    size_t histo_Bytes = NUM_BINS*sizeof(unsigned int)*grid.x;
    ...
    histo_private_kernel<<<grid, block>>>(d_text_data, length, d_histo_gpu);
    cudaDeviceSynchronize();
    cudaMemcpy(h_histo_gpu, d_histo_gpu, histo_Bytes/grid.x, cudaMemcpyDeviceToHost);
    ...
    */
    ...
    printf("Warmup complete for histo_private_kernel\n");
    ...
    printf("Execution time for histo_private_kernel: %f ms\n\n",milliseconds/BENCHMARK_REPEAT);

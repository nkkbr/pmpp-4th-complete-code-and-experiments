...
void histogram_sequential(const char *data, unsigned int length, unsigned int *histo) {
    for (unsigned int i =0;i<length;++i) {
        unsigned char char_index = (unsigned char)data[i];
        histo[char_index]++;
    }
}
...
void print_histo_256(const char* name, unsigned int *histo) {
    printf("========== [%s] (Non-zero Bins) ==========\n", name);
    
    int columns = 0;
    for (int i = 0; i < 256; i++) {
        if (histo[i] == 0) continue; 

        unsigned int count = histo[i] / INPUT_REPETITIONS;
        char sym_buff[16]; 

        if (i == ' ') {
            sprintf(sym_buff, "SPC");
        } else if (i == 127) {
            sprintf(sym_buff, "DEL");
        } else if (isprint(i)) {
            sprintf(sym_buff, "'%c'", (char)i); 
        } else {
            switch (i) {
                case '\n': sprintf(sym_buff, "\\n"); break;
                case '\r': sprintf(sym_buff, "\\r"); break;
                case '\t': sprintf(sym_buff, "\\t"); break;
                case '\0': sprintf(sym_buff, "\\0"); break;
                default:   sprintf(sym_buff, "0x%02X", i); break;
            }
        }

        printf("[%3d | %-5s]: %8u    ", i, sym_buff, count);
        columns++;
        if (columns % 7 == 0) printf("\n");
    }
...
__global__ void histo_private_shared_memory_coarsening_interleaved(char *data, unsigned int length, unsigned int *histo){
    ...
    for(unsigned int i=tid;i<length;i += blockDim.x*gridDim.x){
        unsigned char char_index = (unsigned char)data[i];
        atomicAdd(&(histo_s[char_index]),1);
    }
    __syncthreads();
...
int main(int argc, char **argv){
    ...
    // Print Comparison
    print_histo_256("histo_cpu", h_histo_cpu);
    print_histo_256("histo_gpu", h_histo_gpu);

...
#define NUM_BINS 7
#define INPUT_REPETITIONS 10000
...
__global__ void histo_private_shared_memory_coarsening_interleaved(char *data, unsigned int length, unsigned int *histo){
    ...
    for(unsigned int bin=threadIdx.x;bin<NUM_BINS;bin+=Block_X){
        histo_s[bin]=0u;
    }
    __syncthreads();

    unsigned int tid = blockIdx.x*Block_X+threadIdx.x;

    // Grid-Stride Loop: Allows processing data arrays larger than the total number of threads
    for(unsigned int i=tid;i<length;i += Block_X*gridDim.x){
        int alphabet_position =data[i]-'a';
        if(alphabet_position>=0 && alphabet_position<26){
...
void launch_kernel(int grid_size, int block_size, char *d_text_data, unsigned int length, unsigned int *d_histo_gpu){
    ...
    for (int i=0;i<5;i++){
        switch (block_size) {
            case 32:
                histo_private_shared_memory_coarsening_interleaved<32><<<grid, 32>>>(d_text_data, length, d_histo_gpu);
                break;
            case 64:
                histo_private_shared_memory_coarsening_interleaved<64><<<grid, 64>>>(d_text_data, length, d_histo_gpu);
                break;
            case 128:
                histo_private_shared_memory_coarsening_interleaved<128><<<grid, 128>>>(d_text_data, length, d_histo_gpu);
                break;
            case 256:
                histo_private_shared_memory_coarsening_interleaved<256><<<grid, 256>>>(d_text_data, length, d_histo_gpu);
                break;
            case 512:
                histo_private_shared_memory_coarsening_interleaved<512><<<grid, 512>>>(d_text_data, length, d_histo_gpu);
                break;
            case 1024:
                histo_private_shared_memory_coarsening_interleaved<1024><<<grid, 1024>>>(d_text_data, length, d_histo_gpu);
                break;
            default:
                fprintf(stderr, "Error: Unsupported block size %d\n", block_size);
                exit(1);
        }
...
int main(int argc, char **argv){
    ...
    histo_private_shared_memory_coarsening_interleaved<256><<<grid, block>>>(d_text_data, length, d_histo_gpu);
    cudaDeviceSynchronize();
    ...
    */
    ...
    for (int b : block_sizes) {
        ...
        for (int m : grid_multipliers) {
            launch_kernel(m*numSMs,b,d_text_data, length, d_histo_gpu);
            printf("-------------------------------\n");
        }
    }

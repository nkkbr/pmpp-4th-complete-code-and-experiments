...
#define NUM_BINS 7 // In this code, this number should not be changed! It can only be 7!
...
__global__ void histo_private_shared_memory_coarsening_interleaved(char *data, unsigned int length, unsigned int *histo){
    ...
    unsigned int r0=0,r1=0,r2=0,r3=0,r4=0,r5=0,r6=0;

    unsigned int tid = blockIdx.x*blockDim.x+threadIdx.x;

    // Grid-Stride Loop: Allows processing data arrays larger than the total number of threads
    // No atomicAdd. No shared memory. High Speed.
    for(unsigned int i=tid;i<length;i += blockDim.x*gridDim.x){
        int alphabet_position =data[i]-'a';
        if(alphabet_position>=0 && alphabet_position<26){
            unsigned int bin = alphabet_position/4;
            switch(bin){
                case 0:r0++;break;
                case 1:r1++;break;
                case 2:r2++;break;
    ...
    if (threadIdx.x < NUM_BINS) histo_s[threadIdx.x] = 0;
    __syncthreads();

    atomicAdd(&histo_s[0],r0);
    atomicAdd(&histo_s[1],r1);
    atomicAdd(&histo_s[2],r2);
    atomicAdd(&histo_s[3],r3);
    atomicAdd(&histo_s[4],r4);
    atomicAdd(&histo_s[5],r5);
    atomicAdd(&histo_s[6],r6);

...
__global__ void histo_private_shared_memory_coarsening_contiguous_aggregated(char *data, unsigned int length, unsigned int *histo){
...
int main(int argc, char **argv){
    ...
    dim3 block(256);
    unsigned int total_threads_needed = (length + CFACTOR - 1) / CFACTOR;
    dim3 grid((total_threads_needed + block.x - 1) / block.x);
    printf("grid:  %5d, %5d, %5d\n", grid.x, grid.y, grid.z);
    printf("block: %5d, %5d, %5d\n\n", block.x, block.y, block.z);

    // Ensure the calculated capacity covers the input length.
    printf("Max text length processable with current Grid/Block settings: %u characters\n", grid.x*block.x*CFACTOR); 
    ...
    histo_private_shared_memory_coarsening_contiguous_aggregated<<<grid, block>>>(d_text_data, length, d_histo_gpu);
    ...
    */
    ...
    printf("Warmup complete for histo_private_shared_memory_coarsening_contiguous_aggregated\n");
    ...
    printf("Execution time for histo_private_shared_memory_coarsening_contiguous_aggregated: %f ms\n\n",milliseconds/BENCHMARK_REPEAT);

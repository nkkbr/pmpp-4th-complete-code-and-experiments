...
__global__ void VectorizedCoarsenedShuffleSumReductionKernel(float* input, float* output, unsigned long long N, unsigned long long coarse_factor){
    float4* input_vec = reinterpret_cast<float4*>(input);
    unsigned long long num_vecs = N/4;
    ...
    if (blockIdx.x == 0 && threadIdx.x == 0){
        unsigned long long processed_count = num_vecs * 4;
        for (unsigned long long idx = processed_count; idx<N; ++idx) {
...
void launch_kernel(float* input, float* output ,unsigned int block_dim, unsigned int grid_dim, unsigned long long N, unsigned long long coarse_factor){
    ...
    for (int i=0;i<5;i++){
        switch (block_dim) {
            case 32:
                VectorizedCoarsenedShuffleSumReductionKernel<32><<<grid, block>>>(input,output,N,coarse_factor);
                break;
            case 64:
                VectorizedCoarsenedShuffleSumReductionKernel<64><<<grid, block>>>(input,output,N,coarse_factor);
                break;
            case 128:
                VectorizedCoarsenedShuffleSumReductionKernel<128><<<grid, block>>>(input,output,N,coarse_factor);
                break;
            case 256:
                VectorizedCoarsenedShuffleSumReductionKernel<256><<<grid, block>>>(input,output,N,coarse_factor);
                break;
            case 512:
                VectorizedCoarsenedShuffleSumReductionKernel<512><<<grid, block>>>(input,output,N,coarse_factor);
                break;
            case 1024:
                VectorizedCoarsenedShuffleSumReductionKernel<1024><<<grid, block>>>(input,output,N,coarse_factor);
                break;
            default:
    ...
    printf("Warmup complete for VectorizedCoarsenedShuffleSumReductionKernel\n");
    ...
    printf("Execution time for VectorizedCoarsenedShuffleSumReductionKernel: %f ms\n\n\n",milliseconds/BENCHMARK_REPEAT);
...
int main(int argc, char **argv){
    ...
    constexpr unsigned long long num_vecs = N/4;
    constexpr unsigned long long COARSE_FACTOR = (num_vecs + DIVISOR - 1) / DIVISOR;
    ...
    dim3 block(BLOCK_DIM);
    dim3 grid(GRID_DIM);
    VectorizedCoarsenedShuffleSumReductionKernel<BLOCK_DIM><<<grid, block>>>(d_data, d_result_gpu, N,COARSE_FACTOR);
    ...
    for (unsigned long long n : n_list) {
        ...
        for (unsigned int b : block_sizes) {
            ...
            for ( unsigned int m : grid_multipliers) {
                ...
                unsigned long long coarse_factor = (num_vecs + DIVISOR - 1) / DIVISOR;

...
__global__ void CoarsenedSumReductionKernel(float* input, float* output, unsigned long long N, unsigned long long coarse_factor){
...
void launch_kernel(float* input, float* output ,unsigned int block_dim, unsigned int grid_dim, unsigned long long N, unsigned long long coarse_factor){
    ...
    for (int i=0;i<5;i++){
        switch (block_dim) {
            case 32:
                CoarsenedSumReductionKernel<32><<<grid, block>>>(input,output,N,coarse_factor);
                break;
            case 64:
                CoarsenedSumReductionKernel<64><<<grid, block>>>(input,output,N,coarse_factor);
                break;
            case 128:
                CoarsenedSumReductionKernel<128><<<grid, block>>>(input,output,N,coarse_factor);
                break;
            case 256:
                CoarsenedSumReductionKernel<256><<<grid, block>>>(input,output,N,coarse_factor);
                break;
            case 512:
                CoarsenedSumReductionKernel<512><<<grid, block>>>(input,output,N,coarse_factor);
                break;
            case 1024:
                CoarsenedSumReductionKernel<1024><<<grid, block>>>(input,output,N,coarse_factor);
                break;
            default:
    ...
    printf("Warmup complete for CoarsenedSumReductionKernel\n");
    ...
    printf("Execution time for CoarsenedSumReductionKernel: %f ms\n\n\n",milliseconds/BENCHMARK_REPEAT);
...
int main(int argc, char **argv){
    ...
    dim3 block(BLOCK_DIM);
    dim3 grid(GRID_DIM);
    CoarsenedSumReductionKernel<BLOCK_DIM><<<grid, block>>>(d_data, d_result_gpu, N,COARSE_FACTOR);

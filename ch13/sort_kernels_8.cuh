#ifndef SORT_KERNELS_8_CUH
#define SORT_KERNELS_8_CUH
...
__global__ void radix_sort_coalesced_scatter(
    ...
    // 每个 warp 中，尾数为0，1，2，3,...,7的数字的总和
    __shared__ unsigned int s_warp_counts[NUM_WARPS][8];

    // 对于每个 warp，它之前的（不包含自己）warp 的尾数为0，1，2，3,...,7的数字的总和
    __shared__ unsigned int s_warp_prefix[NUM_WARPS][8];

    // 当前block中，在排列好之后，0，1，2，3,...,7 尾数的数字（我们称为桶），起始的编号是多少
    __shared__ unsigned int s_bucket_base[8];
    ...
    const unsigned int digit = (key >> iter) & 7u;
    ...
    const unsigned int m4 = __ballot_sync(FULL, digit == 4u);
    const unsigned int m5 = __ballot_sync(FULL, digit == 5u);
    const unsigned int m6 = __ballot_sync(FULL, digit == 6u);
    const unsigned int m7 = __ballot_sync(FULL, digit == 7u);
    ...
    if (lane == 0) {
        ...
        s_warp_counts[warpId][4] = __popc(m4);
        s_warp_counts[warpId][5] = __popc(m5);
        s_warp_counts[warpId][6] = __popc(m6);
        s_warp_counts[warpId][7] = __popc(m7);
    }
    ...
    else if (digit == 3u) rank_in_warp = __popc(m3 & lane_lt_mask);
    else if (digit == 4u) rank_in_warp = __popc(m4 & lane_lt_mask);
    else if (digit == 5u) rank_in_warp = __popc(m5 & lane_lt_mask);
    else if (digit == 6u) rank_in_warp = __popc(m6 & lane_lt_mask);
    else  rank_in_warp = __popc(m7 & lane_lt_mask);

    __syncthreads();

    if (tid ==0) {
        unsigned int run0=0, run1=0, run2=0, run3=0, run4=0, run5=0, run6=0, run7=0;
        ...
        for(int w=0; w<NUM_WARPS; ++w){
            ...
            s_warp_prefix[w][4] = run4;
            s_warp_prefix[w][5] = run5;
            s_warp_prefix[w][6] = run6;
            s_warp_prefix[w][7] = run7;
            run0 += s_warp_counts[w][0];
            run1 += s_warp_counts[w][1];
            run2 += s_warp_counts[w][2];
            run3 += s_warp_counts[w][3];
            run4 += s_warp_counts[w][4];
            run5 += s_warp_counts[w][5];
            run6 += s_warp_counts[w][6];
            run7 += s_warp_counts[w][7];
        }
        ...
        const unsigned int num3 = run3;
        const unsigned int num4 = run4;
        const unsigned int num5 = run5;
        const unsigned int num6 = run6;
        ...
        s_bucket_base[4] = num0 + num1 + num2 + num3;
        s_bucket_base[5] = num0 + num1 + num2 + num3 + num4;
        s_bucket_base[6] = num0 + num1 + num2 + num3 + num4 + num5;
        s_bucket_base[7] = num0 + num1 + num2 + num3 + num4 + num5 + num6;
    }
    ...
    const unsigned int b4 = s_bucket_base[4];
    const unsigned int b5 = s_bucket_base[5];
    const unsigned int b6 = s_bucket_base[6];
    const unsigned int b7 = s_bucket_base[7];

    const unsigned int bin = (unsigned int)(tid>=b1) + (unsigned int)(tid>=b2) + (unsigned int)(tid>=b3) + (unsigned int)(tid>=b4) + (unsigned int)(tid>=b5) + (unsigned int)(tid>=b6) + (unsigned int)(tid>=b7);

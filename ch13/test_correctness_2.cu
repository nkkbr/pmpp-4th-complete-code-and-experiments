...
int main() {
    ...
    for (int j=0;j<(M+1);++j){
        ...
        thrust::exclusive_scan(d_block_counts_ptr, d_block_counts_ptr+2*grid_size, d_block_counts_scanned_ptr);
    ...
    verify(h_input.data(), h_gpu_output.data(), N, "radix_sort_2");
    printf("-----------------------\n\n");

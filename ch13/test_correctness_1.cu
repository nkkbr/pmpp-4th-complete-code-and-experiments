...
int main() {
    ...
    unsigned int *d_input, *d_output, *d_bits, *d_ones_before;
    ...
    for (int j=0;j<(M+1);++j){
        ...
        thrust::exclusive_scan(d_bits_ptr, d_bits_ptr+N, d_ones_before_ptr);
    ...
    verify(h_input.data(), h_gpu_output.data(), N, "radix_sort_1");
    printf("-----------------------\n\n");

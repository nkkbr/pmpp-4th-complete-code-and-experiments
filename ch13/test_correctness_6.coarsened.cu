...
int main() {
    ...
    int grid_size = (N+(block_size*coarsen)-1)/(block_size*coarsen);
    ...
    for (int j=0;j<(M+1);j+=2){
        radix_sort_count_block<block_size,coarsen><<<grid_size,block_size>>>(d_in, d_block_counts, j);
        ...
        thrust::exclusive_scan(d_block_counts_ptr, d_block_counts_ptr+4*grid_size, d_block_counts_scanned_ptr);

        radix_sort_coalesced_scatter<block_size,coarsen><<<grid_size,block_size>>>(d_in, d_out, d_block_counts_scanned, j);
    ...
    verify(h_input.data(), h_gpu_output.data(), N, "radix_sort_6");
    printf("-----------------------\n\n");

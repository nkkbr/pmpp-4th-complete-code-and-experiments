...
void sort_kernels_2(
    ...
    for (int j=0;j<(M+1);++j){
        ...
        *time += milliseconds;
        ...
        thrust::exclusive_scan(thrust::cuda::par.on(0), d_block_counts_ptr, d_block_counts_ptr+2*grid_size, d_block_counts_scanned_ptr);
...
int main(int argc, char** argv){
    ...
    cudaMalloc(&d_block_counts, 2*grid_size*sizeof(unsigned int));
    cudaMalloc(&d_block_counts_scanned, 2*grid_size*sizeof(unsigned int));
    ...
    for(int i=0; i<10; ++i) {
        ...
        sort_kernels_2(d_in,d_out,N,d_block_counts,d_block_counts_scanned,grid_size,block_size,&total_time,start,stop);
    }
    ...
    for(int i=0; i<BENCHMARK_REPEAT; ++i) {
        ...
        cudaMemset(d_block_counts, 0, 2*grid_size*sizeof(unsigned int));
        cudaMemset(d_block_counts_scanned, 0, 2*grid_size*sizeof(unsigned int));

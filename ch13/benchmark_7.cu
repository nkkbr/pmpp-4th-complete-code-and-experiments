...
void sort_kernels_7(
    ...
    for (int j=0;j<(M+1);j+=3){
        ...
        *time += milliseconds;
        ...
        thrust::exclusive_scan(thrust::cuda::par.on(0), d_block_counts_ptr, d_block_counts_ptr+8*grid_size, d_block_counts_scanned_ptr);
...
int main(int argc, char** argv){
    ...
    for(int i=0; i<10; ++i) {
        ...
        sort_kernels_7(d_in,d_out,d_block_counts,d_block_counts_scanned,grid_size,block_size,&total_time,start,stop);
    }

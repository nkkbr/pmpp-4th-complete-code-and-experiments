...
__global__ void merge_tiled_kernel(float* A, long long m, float* B, long long n, float* C, unsigned int tile_size){
    ...
    while(counter<total_iteration){
        for(unsigned int i = 0;i<tile_size;i+=blockDim.x){
        ...
        long long a_curr = co_rank(c_curr, A_S, min((long long)tile_size, A_length-A_consumed), B_S, min((long long)tile_size, B_length-B_consumed));
        long long b_curr = c_curr - a_curr;
        long long a_next = co_rank(c_next, A_S, min((long long)tile_size, A_length-A_consumed), B_S, min((long long)tile_size, B_length-B_consumed));
        long long b_next = c_next - a_next;
        ...
        A_consumed += co_rank(tile_size, A_S, current_tile_A_valid, B_S, current_tile_B_valid);
...
void launch_kernel(unsigned int tile_size, unsigned int block_size, unsigned int grid_size, float* d_data_A, float* d_data_B, float* d_data_C, long long A, long long B, long long total_iteration){
    ...
    // 2.1 Warmup
    for (int i=0;i<5;i++){
        merge_tiled_kernel<<<grid, block,shared_mem_bytes>>>(d_data_A, A, d_data_B, B, d_data_C, tile_size);
    }
    cudaDeviceSynchronize();
    printf("Warmup complete for merge_tiled_kernel\n");
    ...
    printf("Execution time for merge_tiled_kernel: %f ms\n\n\n",milliseconds/BENCHMARK_REPEAT);

// 这份文件是直接复制了`ch12_merge_circular_buffer_kernel.cu`，再经过修改得来的，所以有很多的重复的部分
// 虽然不是一个好的做法，不过这里只是为了快速地进行实验
// 这份文件与`ch12_merge_circular_buffer_kernel.cu`的差别仅在于，ch12_merge_circular_buffer_kernel_v2 内部的取模运算，变成了位运算
...
__device__ void merge_sequential_circular(
    ...
    unsigned int tile_mask = tile_size - 1;

    while ((i<m) && (j<n)) {
        long long i_cir = (A_S_start+i)&tile_mask;
        long long j_cir = (B_S_start+j)&tile_mask;
        if (A[i_cir] <= B[j_cir]) {
            C[k++] = A[i_cir];
            ++i;
        } else {
...
__device__ long long co_rank_circular(
    ...
    while(active) {
        long long i_cir = (A_S_start+i)&tile_mask;
        long long i_m_1_cir = (A_S_start+i-1)&tile_mask;
        long long j_cir = (B_S_start+j)&tile_mask;
        long long j_m_1_cir = (B_S_start+j-1)&tile_mask;
...
// 这里 tile_size 事实上都是2的幂，所以，可以将对它的取模运算，转化为位运算
// 通常编译器可以做到这一点，但 tile_size 在这里并不是编译期常量，所以编译器无法进行相应的优化
__global__ void ch12_merge_circular_buffer_kernel_v2(float* A, long long m, float* B, long long n, float* C, unsigned int tile_size){
    ...
    while(counter<total_iteration){
        for(unsigned int i = 0;i<A_S_consumed;i+=blockDim.x){
            if((i+threadIdx.x)<(A_length-A_consumed) && (i+threadIdx.x)<A_S_consumed) {
                A_S[(A_S_start+tile_size-A_S_consumed+i+threadIdx.x)& tile_mask] = A[A_curr+A_consumed+(tile_size - A_S_consumed)+i+threadIdx.x];
            }
        }
        for(unsigned int i = 0;i<B_S_consumed;i+=blockDim.x){
            if((i+threadIdx.x)<(B_length-B_consumed) && (i+threadIdx.x)<B_S_consumed) {
                B_S[(B_S_start+tile_size-B_S_consumed+i+threadIdx.x)& tile_mask] = B[B_curr+B_consumed+(tile_size - B_S_consumed)+i+threadIdx.x];
            }
        }
        __syncthreads();
        ...
        A_S_start = (A_S_start+A_S_consumed) & tile_mask;
        B_S_start = (B_S_start+B_S_consumed) & tile_mask;
        __syncthreads();
    }
}
...
void launch_kernel(unsigned int tile_size, unsigned int block_size, unsigned int grid_size, float* d_data_A, float* d_data_B, float* d_data_C, long long A, long long B, long long total_iteration){
    ...
    // 2.1 Warmup
    for (int i=0;i<5;i++){
        ch12_merge_circular_buffer_kernel_v2<<<grid, block,shared_mem_bytes>>>(d_data_A, A, d_data_B, B, d_data_C, tile_size);
    }
    cudaDeviceSynchronize();
    printf("Warmup complete for ch12_merge_circular_buffer_kernel_v2\n");
    ...
    printf("Execution time for ch12_merge_circular_buffer_kernel_v2: %f ms\n\n\n",milliseconds/BENCHMARK_REPEAT);
